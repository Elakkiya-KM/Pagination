...
let page = document.querySelector(".changes");

let currentItem = 0;
let curPage = 0 / 5 + 1;

// Try to load the data for the first page when the page loads
window.addEventListener("DOMContentLoaded", () => {
  displayEmployees(currentItem);
  page.innerHTML = curPage;
});

// Logic for previous user when the user clicks on the button
prevBtn.addEventListener("click", () => {
  // Decrease the current position by 5.
  if (0 < currentItem) {
    currentItem -= 5;
...
// Logic for the next page when the user clicks on the next button
nextBtn.addEventListener("click", () => {
  // Increase the current position by 5.
  if (w.length - 5 > currentItem) {
    currentItem += 5;
    page.innerHTML = currentItem / 5 + 1;
    displayEmployees(currentItem);
  } else {
...
// Logic for the last button
lastBtn.addEventListener("click", () => {
  if (currentItem < w.length - 5) {
    currentItem = w.length - 5;
    page.innerHTML = currentItem / 5 + 1;
    displayEmployees(currentItem);
  } else {
    alert("You are already on the last page");
...
// Buttons from 1 to 5 code are below:
...
function displayEmployees(startIndex) {
  // Implement the logic to display employees here based on startIndex
}
